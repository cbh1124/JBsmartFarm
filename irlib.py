...
from gpioControl import IR_R, IR_S
...
def sendIRSignal(ir_file_name, btn_code):
    ...
    if not pi.connected:
        print("pigpio connect fail")
        return

    try:
        f = open(ir_file_name, "r")
    except:
        print("Can't open: {}".format(ir_file_name))
        return
    ...
    pi.set_mode(IR_S, pigpio.OUTPUT)  # IR TX connected to this GPIO.
